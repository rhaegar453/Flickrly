...
import {searchGroup} from '../store/Actions/index';
...
class SearchBox extends React.Component {
    ...
    render() {
        return (
            <div>
                <input className="form-control inputBox" value={this.state.searchQuery} placeholder="Search for Groups" onChange={this.changeInputText}></input>
                {this.props.groupRecommendations ? <div className='boxPosition'>
                    {this.props.groupRecommendations.map(item => (
                        <RecommendationItem icon={this.createIconURL(item)} text={item.name}></RecommendationItem>
                    ))}
                </div> : null}
            </div>
        );
